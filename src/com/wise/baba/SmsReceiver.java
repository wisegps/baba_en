...
public class SmsReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			...
			String msg = "";
			String friend_id = "";
			String msg_name = context.getResources().getString(R.string.notice);
			...
			if (isLetter) {
				...
				if (Infos.get(0).topActivity.getPackageName().equals(
						"com.wise.baba")) {
					if (Infos.get(0).topActivity.getClassName().equals(
							"com.wise.notice.LetterActivity")) {
						...
						Intent notificationIntent = new Intent(context,
								NotificationActivity.class); // 点击该通知后要跳转的Activity
						...
						notification.setLatestEventInfo(context, msg_name, msg,
								contentItent);
					...
					notification.defaults |= Notification.DEFAULT_SOUND;
					Intent notificationIntent = new Intent(context,
							WelcomeActivity.class); // 点击该通知后要跳转的Activity
					notificationIntent.putExtras(intent.getExtras());
	...
	private void init(Context context, Bundle bundle) {
		...
		for (int i = 0; i < Infos.size(); i++) {
			if (Infos.get(i).topActivity.getPackageName().equals(
					"com.wise.baba")) {
				isOpen = true;
				// 打开

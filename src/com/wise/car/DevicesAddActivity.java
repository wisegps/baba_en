...
public class DevicesAddActivity extends Activity {
		...
		public void handleMessage(Message msg) {
			...
			case REQUEST_NEAR:
				Toast.makeText(DevicesAddActivity.this,
						R.string.near_share_success, Toast.LENGTH_SHORT).show();
				break;
			case REQUEST_FAR:
				Toast.makeText(DevicesAddActivity.this,
						R.string.far_share_success, Toast.LENGTH_SHORT).show();
				break;
			case getBitmap:
				int type = msg.arg1;
	...
	private void showToast() {
		if (isBind) {
			Toast.makeText(DevicesAddActivity.this,
					R.string.bind_terminal_faild, Toast.LENGTH_SHORT).show();
		} else {
			Toast.makeText(DevicesAddActivity.this,
					R.string.update_terminal_faild, Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void Add() {
		...
		if (serial.equals("")) {
			et_serial.setError(getResources().getString(
					R.string.serial_number_null));
		} else if (sim.length() != 11) {
			et_sim.setError(getResources().getString(R.string.sim_card_error));
		} else {
			if (isBind) {
				tv_note.setText(getResources().getString(
						R.string.terminal_binding));
			} else {
				tv_note.setText(getResources().getString(
						R.string.terminal_updating));
			}
		...
		try {
			if (result.equals("")) {
				...
				if (custID == Integer.valueOf(app.cust_id) || custID == 0) {
					...
					Toast.makeText(DevicesAddActivity.this,
							R.string.haved_terminal, Toast.LENGTH_LONG).show();
					SaveDataOver();
				}
			}
		} catch (Exception e) {
		...
		public void onFocusChange(View v, boolean hasFocus) {
			if (!hasFocus) {
				...
				case R.id.et_sim:
					String sim = et_sim.getText().toString().trim();
					if (sim.length() != 11) {
						et_sim.setError(getResources().getString(
								R.string.sim_card_error));
					}
					break;
				}
			}

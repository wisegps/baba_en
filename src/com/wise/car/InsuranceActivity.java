 ...
 * 保险公司
 */
public class InsuranceActivity extends Activity implements IXListViewListener {
	...
	OnItemClickListener onItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
			...
			if (!isNeedPhone) {
				...
				InsuranceData insuranceData = (InsuranceData) lv_insurance
						.getItemAtPosition(arg2);
				intent.putExtra("insurance_name", insuranceData.getName());
				intent.putExtra("insurance_phone",
	...
	private void getDBData() {
		List<BaseData> baseDatas = DataSupport.where("Title = ?", "insurance")
				.find(BaseData.class);
		...
		if (result == null || result.equals("")) {
			...
			try {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					insuranceData.setService_phone(jsonObject
							.getString("service_phone"));

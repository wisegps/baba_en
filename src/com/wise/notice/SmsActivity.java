...
public class SmsActivity extends Activity implements IXListViewListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
			...
			if (smsDatas != null && arg2 != 0 && arg2 != (smsDatas.size() + 1)) {
				...
				if (Type.equals("0")) {
					...
					// int reminder_id = smsDatas.get(arg2 -
					// 1).getReminder_id();
					// if (reminder_id != 0) {
					// intent.putExtra("Obj_id", smsDatas.get(arg2 - 1)
					// .getObj_id());
					...
					// startActivity(new Intent(SmsActivity.this,
	...
	OnFinishListener onFristFinishListener = new OnFinishListener() {
		...
		public void OnFinish(int index) {
			...
			smsView.getLv_sms().setAdapter(
					new NewAdapter(smsView.getSmsDatas()));
	...
	private void showSms() {
		...
		for (int i = 0; i < 3; i++) {
			...
			View v = LayoutInflater.from(this).inflate(R.layout.item_sms_type,
					null);
			hsl_sms.addView(v);
		...
		if (smsView.getSmsDatas() == null) {
			try {
				...
				String url = Constant.BaseUrl + "customer/" + app.cust_id
						...
						+ "&msg_type=" + index;
				new Thread(new NetThread.GetDataThread(handler, url, frist_sms,
						index_view)).start();
	...
	Handler handler = new Handler() {
		...
		public void dispatchMessage(Message msg) {
			...
			case frist_sms:
				...
				smsViews.get(msg.arg1)
						.setSmsDatas(jsonData(msg.obj.toString()));
				break;
			case GET_SMS:
	...
	public void onRefresh() {
		...
		try {
			if (smsView.getSmsDatas().size() != 0) {
				...
				String url = Constant.BaseUrl + "customer/" + app.cust_id
						+ "/notification?auth_code=" + app.auth_code
						+ "&max_id=" + id + "&msg_type=" + index;
				new Thread(new NetThread.GetDataThread(handler, url, GET_SMS,
						index_view)).start();
	...
	public void onLoadMore() {
		...
		try {
			if (smsView.getSmsDatas().size() != 0) {
				...
				smsView.setLoad("");
				int id = smsView.getSmsDatas()
						.get(smsView.getSmsDatas().size() - 1).getNoti_id();
				String url = Constant.BaseUrl + "customer/" + app.cust_id
						+ "/notification?auth_code=" + app.auth_code
						+ "&min_id=" + id + "&msg_type=" + index;
				/** 区分头尾 **/
				new Thread(new NetThread.GetDataThread(handler, url,
						GET_NEXT_SMS, (index_view + 5))).start();
	...
	private List<SmsData> jsonData(String result) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				String Rcv_time = GetSystem.ChangeTimeZone(jsonObject
						.getString("rcv_time").replace("T", " ")
						.substring(0, 19));
	...
	class NewAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				...
				holder.tv_new_content = (TextView) convertView
						.findViewById(R.id.tv_new_content);
				holder.tv_new_time = (TextView) convertView
						.findViewById(R.id.tv_new_time);
				holder.tv_new_Regnum = (TextView) convertView
						.findViewById(R.id.tv_new_Regnum);

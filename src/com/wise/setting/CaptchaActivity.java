...
public class CaptchaActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		GetCaptcha();
		if (mark == 0) {
			bt_Submit.setText(getResources().getString(R.string.next));
		} else if (mark == 1) {
			if (remove) {
				et_pwd.setVisibility(View.GONE);
				et_pwd_again.setVisibility(View.GONE);
				bt_Submit.setText(getResources().getString(
			...
			bt_Submit.setText(getResources().getString(R.string.update));
			et_pwd_again.setVisibility(View.GONE);
		}
	}
	...
	private void SubmitPhone() {
		...
		if (!sp_pwd.equals(GetSystem.getM5DEndo(pwd))) {
			Toast.makeText(CaptchaActivity.this, R.string.password_error,
					Toast.LENGTH_SHORT).show();
			return;
		}
		if (!captcha.equals(valid_code)) {
			Toast.makeText(CaptchaActivity.this,
					R.string.verification_code_error, Toast.LENGTH_SHORT)
					.show();
			return;
		}
	...
	private void jsonReset(String str) {
		try {
			...
			if (jsonObject.getString("status_code").equals("0")) {
				...
				Toast.makeText(CaptchaActivity.this, R.string.update_faild,
						Toast.LENGTH_SHORT).show();
			}
		} catch (JSONException e) {
			e.printStackTrace();
	...
	private void Submit() {
		...
		if (!pwd.equals(pwdAgain)) {
			Toast.makeText(CaptchaActivity.this, R.string.password_different,
					Toast.LENGTH_SHORT).show();
			return;
		}
		if (!captcha.equals(valid_code)) {

...
public class OilUpdateActivity extends Activity {
	...
	OnClickListener onClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.bt_oil_add_2:// 第二次加油，并录入
				...
				if (oil.equals("")) {
					Toast.makeText(OilUpdateActivity.this, R.string.oil_null,
	...
	private void resetOil() {
		dialog = ProgressDialog.show(OilUpdateActivity.this, getResources()
				.getString(R.string.note),
				getResources().getString(R.string.reset_oil));
		dialog.setCancelable(true);
		String url = Constant.BaseUrl + "device/" + device_id
				+ "/refuel/reset?auth_code=" + app.auth_code;
	...
	private void jsonResetOil(String result) {
		...
		try {
			...
			if (jsonObject.getInt("status_code") == 0) {
				Toast.makeText(OilUpdateActivity.this,
						R.string.reset_oil_success, Toast.LENGTH_SHORT).show();
				bt_oil_add_1.setEnabled(true);
				bt_oil_add_2.setEnabled(false);
			} else {
				Toast.makeText(OilUpdateActivity.this,
						R.string.reset_oil_faild, Toast.LENGTH_SHORT).show();
			}
		} catch (Exception e) {
			e.printStackTrace();
			Toast.makeText(OilUpdateActivity.this, R.string.reset_oil_faild,
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void jsonFristData2Cloud(String result) {
		...
		try {
			...
			if (jsonObject.getInt("status_code") == 0) {
				Toast.makeText(OilUpdateActivity.this,
						R.string.first_oil_success, Toast.LENGTH_SHORT).show();
				bt_oil_add_1.setEnabled(false);
				bt_oil_add_2.setEnabled(true);
			} else {
				Toast.makeText(OilUpdateActivity.this,
						R.string.first_oil_faild, Toast.LENGTH_SHORT).show();
			}
		} catch (Exception e) {
			e.printStackTrace();
			Toast.makeText(OilUpdateActivity.this, R.string.first_oil_faild,
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void jsonSecondData2Cloud(String result) {
		...
		try {
			...
			if (jsonObject.getInt("status_code") == 0) {
				Toast.makeText(OilUpdateActivity.this,
						R.string.second_oil_success, Toast.LENGTH_SHORT).show();
				bt_oil_add_1.setEnabled(true);
				bt_oil_add_2.setEnabled(false);
			} else {
				Toast.makeText(OilUpdateActivity.this,
						R.string.second_oil_faild, Toast.LENGTH_SHORT).show();
			}
		} catch (Exception e) {
			e.printStackTrace();
			Toast.makeText(OilUpdateActivity.this, R.string.second_oil_faild,
					Toast.LENGTH_SHORT).show();
		}
	}
}

...
public class UpdatePwdActivity extends Activity {
	...
	private void updatePwd() {
		...
		if (pwd.equals("") || new_pwd.equals("") || new_pwd_again.equals("")) {
			Toast.makeText(UpdatePwdActivity.this, R.string.complete_info,
					Toast.LENGTH_SHORT).show();
			return;
		}

		if (!new_pwd.equals(new_pwd_again)) {
			Toast.makeText(UpdatePwdActivity.this, R.string.password_different,
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		if (!sp_pwd.equals(GetSystem.getM5DEndo(pwd))) {
			Toast.makeText(UpdatePwdActivity.this, R.string.password_error,
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		params.add(new BasicNameValuePair("field_value", GetSystem
				.getM5DEndo(new_pwd)));
		new Thread(
				new NetThread.putDataThread(handler, url, params, update_pwd))
				.start();
	}
		...
		try {
			...
			if (jsonObject.getString("status_code").equals("0")) {
				...
				Toast.makeText(UpdatePwdActivity.this,
						R.string.password_update_success, Toast.LENGTH_SHORT)
						.show();
				finish();
			} else {
				Toast.makeText(UpdatePwdActivity.this,
						R.string.password_update_faild, Toast.LENGTH_SHORT)
						.show();
			}

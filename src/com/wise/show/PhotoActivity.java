...
public class PhotoActivity extends Activity {
	...
	private void replyPopWin(View v, int position, boolean b) {
		...
		if (photoData.getCust_id().equals(app.cust_id)) {
			// Toast.makeText(PhotoActivity.this, "不能回复自己",
			...
			tv_item_reply.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					tv_people.setText(getResources().getString(R.string.reply)
							+ photoData.getCust_name() + ":");
					mPopupWindow.dismiss();
				}
			});
		...
		for (int i = start; i <= stop; i++) {
			if (start == 0) {
				...
				if (new File(Constant.userIconPath
						+ GetSystem.getM5DEndo(photoDatas.get(i - 1).getIcon())
						+ ".png").exists()) {
		...
		public void run() {
			...
			if (bitmap != null) {
				GetSystem.saveImageSD(
						...
						GetSystem
								.getM5DEndo(photoDatas.get(position).getIcon())
								+ ".png", 100);
			}
	...
	private void sendComments() {
		...
		if (comments.equals("")) {
			Toast.makeText(PhotoActivity.this, R.string.comment_null,
					Toast.LENGTH_SHORT).show();
			return;
		} else {
			progressDialog = ProgressDialog.show(PhotoActivity.this,
					getResources().getString(R.string.note), getResources()
							.getString(R.string.comment_send));
	...
	private void jsonPhoto(String result) {
		try {
			...
			// 读取用户对应的图片
			if (new File(Constant.userIconPath + GetSystem.getM5DEndo(icon)
					+ ".png").exists()) {
				Bitmap image = BitmapFactory.decodeFile(Constant.userIconPath

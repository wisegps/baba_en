	...
	OnClickListener onClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.iv_back:
				...
				break;
			case R.id.tv_series:
				Intent intent = new Intent(ShowCarAcitivity.this,
...
//		new GetLocation(ShowCarAcitivity.this);
	...
	private void checkContent() {
		...
		if (content.equals("")) {
			Toast.makeText(ShowCarAcitivity.this, R.string.describe_null,
					Toast.LENGTH_SHORT).show();
			return;
		}
		if (carBrankId.equals("")) {
			Toast.makeText(ShowCarAcitivity.this, R.string.car_brand_null,
					Toast.LENGTH_SHORT).show();
			return;
		}
		SavaImage();
	...
	private void SavaImage() {
		if (progressDialog == null) {
			progressDialog = ProgressDialog.show(ShowCarAcitivity.this,
					"Data load", getString(R.string.car_uploading));
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				big_pic = app.cust_id + (System.currentTimeMillis() + 1)
						+ ".png";
	...
	private void saveInCloudStorage() {
		...
		params.add(new BasicNameValuePair("car_series", carSeries
				.indexOf(carBrank) >= 0 ? carSeries : (carBrank + carSeries)));
		...
		new NetThread.postDataThread(handler, url, params, postCloudStorage)
				.start();
	}
	...
	private void jsonBrand(String result) {
		...
		// if(jsonObject.opt("car_brand") == null ||
		// jsonObject.opt("car_series") == null){
		// isCustomer = false;
		// }else{
		...
		// getJsonObjectFromJsonArray(result,car_brand);
		// carBrankId = jsonObject2.getString("id");//品牌id;
		// LogUrl = Constant.BaseUrl +"/logo" +
		// jsonObject2.getString("url_icon");//品牌logo地址
		// }
		// }
		// }
		// } catch (Exception e) {
		...
		if (!isCustomer) {
			...
			if (app.carDatas != null || app.carDatas.size() != 0) {
				try {
					...
					JSONObject jsonObject2 = getJsonObjectFromJsonArray(result,
							car_brand);
					LogUrl = Constant.BaseUrl + "logo/"
							+ jsonObject2.getString("url_icon");// 品牌logo地址
				} catch (Exception e) {
	...
	/** 返回对应的JsonObject **/
	private JSONObject getJsonObjectFromJsonArray(String result,
			String car_brand) {
		try {

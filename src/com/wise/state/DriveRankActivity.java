...
public class DriveRankActivity extends Activity {
	...
	private void getData(String data) {
		String url = Constant.BaseUrl + "device/drive_rank/" + data
	...
	class FuelAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			viewHolder.tv_est.setText(getString(R.string.mileage)
					+ driveData.getMileage()
					+ getString(R.string.average_score)
					+ driveData.getDriver_score());
			viewHolder.tv_avg.setText(driveData.getCar_type() + " ("
					+ driveData.getEst_fuel() + "L/100km)");
			// 读取用户对应的图片
			if (new File(Constant.userIconPath
					+ GetSystem.getM5DEndo(driveData.getLogo()) + ".png")
					.exists()) {
			...
			viewHolder.iv_letter.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if (driveData.getCust_id() != Integer.valueOf(app.cust_id)) {
						Intent intent = new Intent(DriveRankActivity.this,
	...
	private void getPersionImage() {
		...
		for (int i = start; i <= stop; i++) {
			...
			if (driveDatas.get(i).getLogo() == null
					|| driveDatas.get(i).getLogo().equals("")) {

			} else {
				// 判断图片是否存在
				if (new File(Constant.userIconPath
						+ GetSystem.getM5DEndo(driveDatas.get(i).getLogo())
						+ ".png").exists()) {
	...
	class ImageThread extends Thread {
		...
		public void run() {
			super.run();
			Bitmap bitmap = GetSystem.getBitmapFromURL(driveDatas.get(position)
					.getLogo());
			if (bitmap != null) {
				GetSystem.saveImageSD(
						...
						GetSystem
								.getM5DEndo(driveDatas.get(position).getLogo())
								+ ".png", 100);
			}

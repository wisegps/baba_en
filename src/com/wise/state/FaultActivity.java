...
public class FaultActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (isSpecify) {
			Intent intent = new Intent(FaultActivity.this, SetActivity.class);
		...
		redDot = (ImageView)findViewById(R.id.dddddd);//红点

		findViewById(R.id.bt_home).setOnClickListener(onClickListener);
		findViewById(R.id.bt_map).setOnClickListener(onClickListener);
		findViewById(R.id.bt_info_first).setOnClickListener(onClickListener);
		findViewById(R.id.bt_set_first).setOnClickListener(onClickListener);
...
//		new GetLocation(FaultActivity.this);
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		startActivityForResult(
				new Intent(FaultActivity.this, SetActivity.class), 5);
		return true;
	}
	...
	OnClickListener onClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.iv_update_oil:
				...
				// R.string.no_bind_no_fuel_update, Toast.LENGTH_SHORT)
				// .show();
				// } else {
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case SHOW_LOCATION:				
				...
				carViews.get(index).getTv_adress().setText(currentPosition);
				//获取焦点释放焦点
				carViews.get(index).getTv_adress().setFocusable(true);
			...
			case get_health:
				...
				try {
					...
					carViews.get(msg.arg1).getTv_title()
			...
			if (index < app.carDatas.size()) {
				...
				if (device_id == null || device_id.equals("")) {
					...
					String url = Constant.BaseUrl + "device/" + device_id
							...
							// "&gas_no="
							+ URLEncoder.encode("", "UTF-8") + "&gas_no="
							+ Gas_no;
	...
	private void jsonGps(String str, int index) {
		try {
			if (index < app.carDatas.size()) {
				...
				System.out.println("获取汽车位置" + "lat = " + lat + " , Lon = " + lon);
				
//				showGeoCodeLocation(latLng);//显示位置.........................
				//获取地理位置信息.........................
				new Geocoding.GetGoogleGeocoding(handler, latLng, SHOW_LOCATION).start();
...
//	}
		...
		try {
			...
			if (index < carViews.size()) {
				...
				if ((jsonObject.getString("left_distance")).equals("null")) {
					...
					carView.getTv_current_distance().setText(
							getString(R.string.remain_mileage));
					try {
						carView.getTv_distance().setText(
								String.format("%.1f",
	...
	private void initDataView() {// 布局
		...
		System.out.println("O(∩_∩)O哈！" + app.carDatas.size());		
		SharedPreferences preferences = getSharedPreferences(
				Constant.sharedPreferencesName, Context.MODE_PRIVATE);
		...
		for (int i = 0; i < app.carDatas.size(); i++) {
			...
			View view = LayoutInflater.from(this).inflate(R.layout.item_fault,
					null);
						
			hs_car.addView(view);
			ImageView iv_update_oil = (ImageView)view
					.findViewById(R.id.iv_update_oil);			
			iv_update_oil.setOnClickListener(onClickListener);			
			RelativeLayout rl_left_complete = (RelativeLayout) view
			...
			TasksCompletedView mTasksView = (TasksCompletedView) view
					.findViewById(R.id.tasks_view);
			mTasksView.setOnClickListener(onClickListener);
			// 当前里程数
			TextView tv_current_distance = (TextView) view
					.findViewById(R.id.tv_current_distance);
			TextView tv_distance = (TextView) view.findViewById(R.id.tv_distance);		
			TextView tv_fee = (TextView) view.findViewById(R.id.tv_fee);
			TextView tv_fuel = (TextView) view.findViewById(R.id.tv_fuel);
			TextView tv_name = (TextView) view.findViewById(R.id.tv_name);			
			//反解析汽车地理位置
			tv_adress = (TextView) view.findViewById(R.id.tv_geocoding);
//			tv_adress.setText("夕阳无限好，只是近黄昏。。。。。。。。。。。");
			view.findViewById(R.id.Liner_distance).setOnClickListener(
					onClickListener);
			view.findViewById(R.id.Liner_fuel).setOnClickListener(onClickListener);
			view.findViewById(R.id.Liner_fee).setOnClickListener(onClickListener);
			TasksCompletedView tcv_drive = (TasksCompletedView) view
					.findViewById(R.id.tcv_drive);
			tcv_drive.setOnClickListener(onClickListener);
			TextView tv_drive = (TextView) view.findViewById(R.id.tv_drive);
			...
			tv_name.setText(app.carDatas.get(i).getNick_name());//车名字		
			String Device_id = app.carDatas.get(i).getDevice_id();
			if (Device_id == null || Device_id.equals("")) {
				carView.getmTasksView().setProgress(100);
				tv_score.setText("0");
				tv_title.setText(getString(R.string.no_bind_terminal));
...
//		showGeoCodeLocation(latLng);//显示位置.........................	
	...
	private void setLoginView() {
		...
		tv_name.setText(getString(R.string.bind_baba_accessory));
	}
	...
	// carView.getTv_xx().setText("");
	// app.carDatas.get(index).setLimit("");
	// } else {
	// JSONObject jsonObject = new JSONObject(result);

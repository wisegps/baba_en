...
//		});
		...
		if (device_id == null || device_id.equals("")) {
			...
			try {
				...
				String url = Constant.BaseUrl + "device/" + device_id
						...
						+ URLEncoder.encode("", "UTF-8") + "&gas_no=93#(92#)";
		...
		for (int i = 0; i < app.carDatas.size(); i++) {
			...
			if (result.equals("")) {// 未体检过
				...
				try {
					...
					tv_title.setText(getString(R.string.before_medical));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void jsonVersion(String result) {
		try {
			double Version = Double.valueOf(GetSystem.GetVersion(
					FaultTestActivity.this, Constant.PackageName));
	...
	private void initView() {// TODO 布局
		...
		View viewCar = LayoutInflater.from(this).inflate(
				R.layout.item_fault_car, null);
		ll_fault_content.addView(viewCar);
		View viewNstv = LayoutInflater.from(this).inflate(
				R.layout.item_fault_nstv, null);
		ll_fault_content.addView(viewNstv);
		View viewWeather = LayoutInflater.from(this).inflate(
				R.layout.item_fault_weather, null);
		ll_fault_content.addView(viewWeather);
		View viewHappy = LayoutInflater.from(this).inflate(
				R.layout.item_fault_happy, null);
	...
	private void setData() {
		...
		viewData0.setId(0);
		viewData0.setName(getString(R.string.car_info));
		viewDatas.add(viewData0);

		ViewData viewData1 = new ViewData();
		viewData1.setId(0);
		viewData1.setName(getString(R.string.roll_info));

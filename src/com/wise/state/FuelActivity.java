...
/*	CarData carData;*/
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (type == FaultActivity.FUEL) {
			tv_chart_title
					.setText(getString(R.string.hundred_fuel_month_curve));
		} else if (type == FaultActivity.DISTANCE) {
			tv_chart_title.setText(getString(R.string.mileage_month_curve));
			tv_chart_unit.setText("km");
		} else if (type == FaultActivity.FEE) {
			tv_chart_title.setText(getString(R.string.cost_month_curve));
			tv_chart_unit.setText("￥");
		} else {
			tv_chart_title.setText(getString(R.string.no_consumption));
		}
		...
		fv.setOnViewRotateListener(new OnViewRotateListener() {
			...
			public void viewRotate(int rotateRanges) {
				...
				if (type == FaultActivity.FUEL) {
					...
					tv_speed_avg_fuel.setText(getString(R.string.average_fuel)
							+ rangeData.getAvg_fuel());
					tv_speed_fuel.setText(getString(R.string.fuel_consumption)
							+ rangeData.getFuel());
				} else if (type == FaultActivity.DISTANCE) {
					tv_speed_text.setText(rangeData.getSpeed_text());
					tv_speed_avg_fuel.setText(getString(R.string.mileage)
							+ rangeData.getDistance());
					tv_speed_fuel.setText(getString(R.string.fuel_consumption)
							+ rangeData.getFuel());
				} else {
					tv_speed_text.setText(rangeData.getSpeed_text());
					tv_speed_avg_fuel.setText(getString(R.string.cost)
							+ rangeData.getFee());
					tv_speed_fuel.setText(getString(R.string.fuel_consumption)
							+ rangeData.getFuel());
				}
		...
		public void onClick(View v) {
			...
			case R.id.tv_month:
				...
				if (type == FaultActivity.FUEL) {
					...
					tv_chart_title
							.setText(getString(R.string.mileage_month_curve));
					tv_chart_unit.setText("km");
				} else if (type == FaultActivity.FEE) {
					tv_chart_title
							.setText(getString(R.string.cost_month_curve));
					tv_chart_unit.setText("￥");
				} else {
					tv_chart_title.setText(getString(R.string.no_consumption));
				}
			...
			case R.id.tv_week:
				...
				if (type == FaultActivity.FUEL) {
					tv_chart_title
							.setText(getString(R.string.hundred_fuel_week_curve));
				} else if (type == FaultActivity.DISTANCE) {
					tv_chart_title
							.setText(getString(R.string.mileage_week_curve));
					tv_chart_unit.setText("km");
				} else if (type == FaultActivity.FEE) {
					tv_chart_title.setText(getString(R.string.cost_week_curve));
					tv_chart_unit.setText("￥");
				} else {
					tv_chart_title.setText(getString(R.string.no_consumption));
				}
			...
			case R.id.iv_right:
				if (index == 2) {
					...
					getData(weekData[0], weekData[6]);
					boolean isMax = GetSystem.maxTime(
							weekData[6] + " 00:00:00", GetSystem.GetNowMonth()
									.getDay() + " 00:00:00");
					if (isMax) {
						iv_right.setVisibility(View.GONE);
					}
	...
	private void jsonData(String str) {
		try {
			...
			// 根据类型分别显示相应的界面
			if (type == FaultActivity.FUEL) {
				tv_title_1.setText(getString(R.string.total_fuel));
				tv_title_3.setText(getString(R.string.total_cost));
				...
				tv_title_map
						.setText(getString(R.string.hundred_average_fuel_map));

			} else if (type == FaultActivity.DISTANCE) {
				tv_title_1.setText(getString(R.string.total_driving));
				tv_title_2.setText(getString(R.string.total_cost));
				tv_content_1.setTextSize(30);
				...
				tv_title_map.setText(getString(R.string.mileage_fuel_map));

			} else if (type == FaultActivity.FEE) {
				tv_distance.setText(total_distance);
				tv_fuel.setText(total_fuel);
				tv_money.setText(total_fee);
				tv_title_map.setText(getString(R.string.cost_fuel_map));
			}
			...
			if (index != 0) {
				...
				try {
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						int rcv_day = Integer.valueOf(jsonArray
								...
								.substring(8, 10));
						String weekDate = jsonArray.getJSONObject(i)
								.getString("rcv_day").substring(0, 10);
				...
				if (index == 1) {
					...
					for (int i = 0; i < 7; i++) {
						...
						if (Efuel.size() <= i) {
							Efuel.add(i, new EnergyItem(day, 0, weekData[i]));
						} else if (!Efuel.get(i).getWeekDate()
								.equals(weekData[i])) {
							Efuel.add(i, new EnergyItem(day, 0, weekData[i]));
						}
					}

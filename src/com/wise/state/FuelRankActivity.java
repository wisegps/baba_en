...
public class FuelRankActivity extends Activity {
	...
	private void getData(String data) {
		String url = Constant.BaseUrl + "device/fuel_rank/" + data
	...
	class FuelAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				viewHolder.tv_est = (TextView) convertView
						.findViewById(R.id.tv_est);
				viewHolder.tv_name = (TextView) convertView
						.findViewById(R.id.tv_name);
				viewHolder.tv_level = (TextView) convertView
						.findViewById(R.id.tv_level);
				viewHolder.iv_icon = (CircleImageView) convertView
						.findViewById(R.id.iv_icon);
				viewHolder.iv_letter = (ImageView) convertView
						.findViewById(R.id.iv_letter);
			...
			viewHolder.tv_est.setText(getString(R.string.mileage)
					+ fuelData.getMileage() + "km "
					+ getString(R.string.fuel_consumption)
					+ fuelData.getAvg_fuel() + "L/100km");
			viewHolder.tv_avg.setText(fuelData.getCar_type() + " ("
					+ fuelData.getEst_fuel() + "L/100km)");
			// 读取用户对应的图片
			if (new File(Constant.userIconPath
					+ GetSystem.getM5DEndo(fuelData.getLogo()) + ".png")
					.exists()) {
			...
			viewHolder.iv_letter.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if (fuelData.getCust_id() != Integer.valueOf(app.cust_id)) {
						Intent intent = new Intent(FuelRankActivity.this,
	...
	private void getPersionImage() {
		...
		for (int i = start; i <= stop; i++) {
			...
			if (fuelDatas.get(i).getLogo() == null
					|| fuelDatas.get(i).getLogo().equals("")) {

			} else {
				// 判断图片是否存在
				if (new File(Constant.userIconPath
						+ GetSystem.getM5DEndo(fuelDatas.get(i).getLogo())
						+ ".png").exists()) {
	...
	class ImageThread extends Thread {
		...
		public void run() {
			super.run();
			Bitmap bitmap = GetSystem.getBitmapFromURL(fuelDatas.get(position)
					.getLogo());
			if (bitmap != null) {
				GetSystem.saveImageSD(bitmap, Constant.userIconPath,
						GetSystem.getM5DEndo(fuelDatas.get(position).getLogo())
								+ ".png", 100);
			}

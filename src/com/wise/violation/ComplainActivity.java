...
public class ComplainActivity extends Activity implements IXListViewListener {
	...
	private void send() {
		...
		if (content.equals("")) {
			Toast.makeText(ComplainActivity.this, R.string.content_null,
					Toast.LENGTH_SHORT).show();
			return;
		}
		String url = Constant.BaseUrl + "violation_complain?auth_code="
				+ app.auth_code;
	...
	private List<ComplainData> jsonData(String str) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				ComplainData complainData = new ComplainData();
				complainData.setCreate_time(GetSystem.ChangeTimeZone(jsonData
						.getString("create_time").substring(0, 19)
						.replace("T", " ")));
	...
	private void getData() {
		try {
			ll_wait.startWheel();
			String url = Constant.BaseUrl + "violation_complain/"
					+ URLEncoder.encode(Location, "UTF-8") + "?auth_code="
					+ app.auth_code;
	...
	class ComplainAdapter extends BaseAdapter {
		...
		public View getView(int arg0, View convertView, ViewGroup arg2) {
			...
			if (complainDatas.get(arg0).getCust_name().equals("")) {
				holder.tv_name.setText(getString(R.string.anonymous));
			} else {
	...
	public void onLoadMore() {
		if (complainDatas.size() != 0) {
			String id = complainDatas.get(complainDatas.size() - 1)
					.getComplain_id();
			try {
				load = "";
				String url = Constant.BaseUrl + "violation_complain/"
						+ URLEncoder.encode(Location, "UTF-8") + "?auth_code="
						+ app.auth_code + "&min_id=" + id;
				new Thread(new NetThread.GetDataThread(handler, url, LoadData))
						.start();
